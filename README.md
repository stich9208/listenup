...
- 녹음 음량 확인 및 기존 오디오 가져오기
- 회의·강의에 맞춘 한국어 전사와 요약
- 전사와 요약을 확인하고 필요한 내용 복사
...
`main`에 push하면 GitHub Actions가 테스트·DMG 빌드 후 [최신 베타 Release](https://github.com/stich9208/listenup/releases/tag/latest)를 자동 갱신합니다. 배포 버전을 고정하려면 `Info.plist` 버전을 변경하고 `v*` 태그를 push하세요.

## 사용 방법

1. 설정에서 OpenAI API 키를 지정합니다.
2. 녹음 제목·용도·대상을 선택하고 녹음을 시작합니다.
3. 녹음을 마친 뒤 OpenAI 처리를 실행합니다.
4. 전사와 요약을 확인하거나 **결과물 내보내기**를 선택합니다.

앱 소리를 처음 녹음할 때는 **화면 및 시스템 오디오 녹음** 권한을 허용한 뒤 ListenUp을 완전히 종료하고 다시 열어야 앱 목록이 표시됩니다. 설정에서 이미 ListenUp이 켜져 있는데도 목록이 없으면 권한을 한 번 껐다가 다시 켜고 앱을 재실행하세요.

내보낸 ZIP에는 다음 두 파일만 포함됩니다.

- `recording.m4a`
- `transcript-summary.html` — 선택·복사할 수 있는 요약과 전체 전사

세션 정보, 내부 JSON, 로그, API 요청·응답 및 API 키는 결과물에 포함되지 않습니다.

녹음과 처리에 필요한 작업 파일은 앱 내부 저장소에 보관됩니다. 사용자가 **결과물 내보내기**를 선택하기 전에는 데스크탑이나 문서 폴더에 파일 또는 세션 디렉터리를 만들지 않습니다.
