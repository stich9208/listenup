...
6. 보안 영역에서 ListenUp의 **그래도 열기**를 누릅니다.
...
시스템 설정의 **개인정보 보호 및 보안 → 화면 및 시스템 오디오 녹음**에서 ListenUp이 이미 켜져 있는데도 앱 목록이 나타나지 않으면 다음 순서로 권한을 다시 등록합니다.

1. ListenUp을 종료합니다.
2. 시스템 설정에서 ListenUp을 끕니다.
3. ListenUp을 다시 켭니다.
4. ListenUp을 실행합니다.

이 베타는 Developer ID 서명 없이 배포되므로, 기존 설치본의 권한이 이전 빌드에 남아 있는 경우 한 번 재등록해야 할 수 있습니다.
